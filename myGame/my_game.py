...
# The text that will show up on the screen 
...
# The details to define the player and what they can do and how they do it 
class Player(Sprite):
...
# The main platforms that the player can stand on in the game 
...
# The aliens that will attack you 
class Mob(Sprite):
    def __init__(self, x, y, w, h, category):
        Sprite.__init__(self)
        self.image = pg.Surface((w, h))
# What they look like 
        self.image = pg.image.load(os.path.join(img_folder, 'robot.png')).convert()
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
        print(self.rect.center)
        ...
        self.speed = 1
    def update(self):
# Letting them love around in a predictable pattern 
        if self.category == "moving":
            self.rect.x += self.speed
            if self.rect.x + self.rect.w > WIDTH or self.rect.x < 0:
                self.speed = -self.speed
...
# creating the window for the game so that it can run
pg.init()
pg.mixer.init()
...
# add instance for classes 
player = Player()
# add instance for groups 
all_sprites.add(player)

# Making it so that the platforms spawn in random locations in an area 
for i in range (0, 15):
    p = Platform(randint(0,WIDTH),randint(0,HEIGHT),30,30,"bounce")
    all_sprites.add(p)
    all_platforms.add(p)

# Adding it so that mobs spawn in ramdon locations and also respawn 
...
# Score for if the mobs are killed 
    mhits = pg.sprite.spritecollide(player, all_mobs, True)
    if mhits:
        mob_count -= 1
        SCORE += 1
    
# Having the mobs respawn 
    while mob_count <= 20:
        m = Mob(randint(0,WIDTH),randint(0,HEIGHT),25,25,"moving")
        all_sprites.add(m)
        all_mobs.add(m)
